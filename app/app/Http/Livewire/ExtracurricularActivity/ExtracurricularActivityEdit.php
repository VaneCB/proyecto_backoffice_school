...
class ExtracurricularActivityEdit extends Component
{
    ...
    public function save()
    {
        ...
        if ($this->activity->id) {
            // Verificar si el material asignado previamente es diferente del nuevo material
            if ($this->activity->material_id && $this->activity->material_id !== $this->activity->getOriginal('material_id')) {
                // Obtener el material previamente asignado
                $previousMaterial = Material::find($this->activity->getOriginal('material_id'));

                if ($previousMaterial) {
                    // Si hay un material anterior asignado, devolverlo (sumar 1 al stock)
                    $previousMaterial->stock += 1;
