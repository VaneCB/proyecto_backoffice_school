...
<div class="column ">
    ...
    <div class="avatar avatar-xl position-relative">
        <a class="align-items-center d-flex m-0 navbar-brand text-wrap" href="{{ route('profile', ['id' => $user->id]) }}">
            @if(Auth::check() && Auth::user()->photo)
                <img src="{{ asset('storage/' . Auth::user()->photo) }}" alt="User Photo" class="w-150 border-radius-lg shadow-sm nav-link text-body p-0">
            @else
                <img src="../assets/img/bruce-mars.jpg" alt="Default User Photo" class="w-150 border-radius-lg shadow-sm nav-link text-body p-0">
            @endif
        </a>
    </div>
    ...
    <li class="nav-item px-3 d-flex align-items-center">
        <a class=" nav-link text-body p-0
                        {{ in_array(request()->route()->getName(),['profile', 'my-profile']) ? 'text-white' : '' }}" wire:click="logout">
            <i class="fa fa-sign-out text-dark  fixed-plugin-button-nav cursor-pointer" style="font-size:24px"></i>
        </a>
    </li>
    </ul>
</div>
    <div class="position-relative p-2">
        <h6 class="font-weight-bolder text-dark text-gradient">  {{$user->name}}</h6>
    </div>
</div>
{{--</div>
    <div class="column">
